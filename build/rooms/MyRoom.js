...
class MyRoom extends colyseus_1.Room {
    ...
    onCreate() {
        ...
        this.onMessage("animation_completed", (client, message) => {
            ...
            const playersWhoRolledThisRound = Array.from(this.state.players.values())
                .filter((p) => p.history.length === this.state.currentRound); // सुनिश्चित करें कि इस राउंड में रोल किया हो
            const allActivePlayersCompletedAnimation = playersWhoRolledThisRound
                .every((p) => this.state.animationCompletedFlags.get(p.sessionId) === true);
            if (allActivePlayersCompletedAnimation) {
                ...
                const allRolledFinalTurns = Array.from(this.state.players.values())
                    .every((p) => p.history.length === this.TOTAL_TURNS); // 'p' को Player टाइप दिया
                if (allRolledFinalTurns) {
                    ...
                    const totalRollsCount = Array.from(this.state.players.values()).reduce((sum, p) => sum + p.history.length, 0); // sum और p को टाइप दिया
    ...
    onJoin(client, options) {
        ...
        player.history = new Array(); // खाली हिस्ट्री एरे
